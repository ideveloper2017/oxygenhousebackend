    ...
    console.time()
      ...
      for (let entrance = 1; entrance <= createBuildingDto.entrance_number; entrance++) {
        for (let floor = 1; floor <= createBuildingDto.floor_number; floor++) {
          for (let apartment = 1; apartment <= createBuildingDto.apartment_number; apartment++) {
            let apartment = new Apartments()
      ...
      const result = await this.buildingRepository.manager.getRepository(Apartments).save(records);
